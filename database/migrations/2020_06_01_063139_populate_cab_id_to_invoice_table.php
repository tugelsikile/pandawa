...
class PopulateCabIdToInvoiceTable extends Migration
{
    /**
     * Run the migrations.
     *
                ...
                if (!collect($members)->isEmpty()){
                    foreach ($members as $keyM => $member){
                        ...
                        if (!collect($customer)->isEmpty()){
                            DB::table('isp_invoice')->where(['status'=>1,'cust_id'=>$member->cust_id])->update(array('cab_id' => $cabang->cab_id));
                        }
                    }
                }
            }
    ...
    {
        DB::table('isp_invoice')->update(array('cab_id' => null));
    }
}
