...
<div class="form-group row">
    ...
    <div class="col-sm-10">
        <input type="text" class="form-control" id="nama_pengguna" name="nama_pengguna" value="{{$data->name}}">
    </div>
</div>
<div class="form-group row">
        ...
        <select name="level_pengguna" id="level_pengguna" class="form-control">
            @if($user_levels)
                @foreach($user_levels as $key => $level)
                    <option @if($level->id == $data->level_id->id) selected @endif value="{{ $level->id }}">{{ $level->name }}</option>
                @endforeach
            @endif
        </select>
    </div>
...
<script>
    ...
    $('#ModalForm').attr({'action':'{{ url('radius-server/update') }}'});
</script>