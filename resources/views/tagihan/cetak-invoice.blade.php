            ...
            <table class="it-grid" width="100%">
                ...
                @foreach($page as $invoice)
                    ...
                    <tr>
                        ...
                        <td>{{ $invoice->fullname }}<br><small>{{ $invoice->inv_number }}</small></td>
                        <td>{{ bulanIndo(date('m',strtotime($invoice->inv_date))) }} {{ date('Y',strtotime($invoice->inv_date)) }}</td>
                        <td>{{ format_rp($invoice->price) }}</td>
                        <td>{{ format_rp($invoice->price_with_tax - $invoice->price) }}</td>
                        <td>{{ format_rp($invoice->price_with_tax) }}</td>
                        <td>{{ $invoice->is_paid == 1 ? 'Lunas' : '-' }}</td>
                    </tr>
                    <?php $nomor++ ?>
                @endforeach
                </tbody>
